...
Then('the publisher should be on live stream page', async function (this: ScenarioWorld) {
  liveStreamPage = new LiveStreamPage(this.publisherPage);
  await liveStreamPage.waitForPageLoad();
});

Then('the publisher video should be on shown in video frame', async function (this: ScenarioWorld) {
